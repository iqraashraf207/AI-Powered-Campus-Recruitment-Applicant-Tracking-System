async function loadDashboard() {
    try {
      ...
      if (recs.length === 0) {
        ...
        recEl.innerHTML = recs.slice(0, 3).map(function(r) {
          const fit = parseFloat(r.fit_score);
          return '<div style="display:flex; align-items:center; justify-content:space-between;' +
                 'padding:10px 0; border-bottom:1px solid #f1f5f9;">' +
                 '<div style="flex:1;">' +
                 '<div style="font-size:13px; font-weight:600; color:#1e293b;">' + r.title + '</div>' +
                 '<div style="font-size:12px; color:#64748b;">' + r.company_name + '</div>' +
  ...
  async function loadJobs() {
    try {
      ...
      showAlert('alert-jobs', 'Could not load jobs. Make sure the backend is running.', 'error');
    }
  }
  ...
  function renderJobs(jobs) {
    ...
    tbody.innerHTML = jobs.map(function(j) {
      ...
      return '<tr>' +
        ...
        '<td>' +
        '<a href="/student/job_detail.html?id=' + j.job_id + '"' +
        ' class="btn btn-primary btn-sm">View & Apply</a>' +
        '</td>' +
        '</tr>';
    }).join('');
  }
  ...
  function filterJobs() {
    ...
    try {
      ...
      if (!res.ok) {
        showAlert('alert-detail', 'Job not found.', 'error');
        return;
      }
      ...
      document.getElementById('job-detail-card').innerHTML =
        ...
        job.company_name + ' · ' + (job.location || 'Pakistan') + '</p>' +
        '</div>' +
        ...
        ' letter-spacing:0.5px; margin-bottom:4px;">Salary</div>' +
        '<div style="font-size:16px; font-weight:700; color:#1e293b;">' +
        'Rs ' + Number(job.salary).toLocaleString() + '</div></div>' +
        '<div style="background:#f8fafc; border-radius:8px; padding:14px;">' +
        '<div style="font-size:11px; color:#64748b; text-transform:uppercase;' +
        ' letter-spacing:0.5px; margin-bottom:4px;">Min CGPA</div>' +
        '<div style="font-size:16px; font-weight:700; color:#1e293b;">' +
        job.min_cgpa + '</div></div>' +
        '<div style="background:#f8fafc; border-radius:8px; padding:14px;">' +
        '<div style="font-size:11px; color:#64748b; text-transform:uppercase;' +
        ' letter-spacing:0.5px; margin-bottom:4px;">Deadline</div>' +
        '<div style="font-size:16px; font-weight:700; color:#1e293b;">' +
        deadline + '</div></div>' +
        '</div>' +
  
        '<div style="margin-bottom:20px;">' +
        '<h3 style="font-size:15px; font-weight:600; color:#1e293b; margin-bottom:10px;">' +
        'Job Description</h3>' +
        ...
        (required.length > 0 ?
          ...
          '<h3 style="font-size:15px; font-weight:600; color:#1e293b; margin-bottom:10px;">' +
          'Required Skills</h3>' +
          required.map(function(s) {
            return '<span class="skill-tag required">' + s.skill_name + '</span>';
          }).join('') + '</div>' : '') +
        ...
        (preferred.length > 0 ?
          ...
          '<h3 style="font-size:15px; font-weight:600; color:#1e293b; margin-bottom:10px;">' +
          'Preferred Skills</h3>' +
          preferred.map(function(s) {
            return '<span class="skill-tag">' + s.skill_name + '</span>';
          }).join('') + '</div>' : '') +
  ...
  async function applyForJob(jobId) {
    try {
      ...
      if (!res.ok) {
        showAlert('alert-apply', data.detail || 'Could not apply.', 'error');
        return;
      }
    ...
    try {
      ...
      if (apps.length === 0) {
        tbody.innerHTML =
          ...
          '<a href="/student/jobs.html" style="color:#1a56db; font-weight:600;">Browse Jobs</a>' +
          '</td></tr>';
        return;
      }
      ...
      tbody.innerHTML = apps.map(function(a) {
        ...
        const score = a.match_score != null
          ? '<div class="score-bar-wrap">' +
            ...
            a.match_score + '%;"></div></div>' +
            '<span class="score-text">' + a.match_score + '</span></div>'
          : '—';
        return '<tr>' +
    ...
    try {
      ...
      if (!data.recommendations || data.recommendations.length === 0) {
        el.innerHTML =
          ...
          '<a href="/student/profile.html" class="btn btn-primary btn-sm">' +
          'Complete Your Profile</a></div></div>';
        return;
      }
  
      el.innerHTML = data.recommendations.map(function(r, i) {
        const fit      = parseFloat(r.fit_score);
        const deadline = new Date(r.deadline).toLocaleDateString('en-GB', {
          day: 'numeric', month: 'short', year: 'numeric'
        });
        return '<div class="card" style="margin-bottom:16px;">' +
          ...
          '<span style="background:#1a56db; color:white; width:26px; height:26px;' +
          ' border-radius:50%; display:flex; align-items:center; justify-content:center;' +
          ' font-size:12px; font-weight:700; flex-shrink:0;">' + (i + 1) + '</span>' +
          '<h3 style="font-size:16px; font-weight:700; color:#1e293b;">' + r.title + '</h3>' +
          '</div>' +
          '<p style="font-size:13px; color:#64748b; margin-bottom:12px;">' +
          r.company_name + ' · Deadline: ' + deadline +
          ' · Rs ' + Number(r.salary).toLocaleString() + '</p>' +
          '<div class="score-bar-wrap">' +
          ...
          fit.toFixed(1) + '% match</span>' +
          '</div>' +
          '</div>' +
          '<a href="/student/job_detail.html?id=' + r.job_id + '"' +
          ' class="btn btn-primary btn-sm" style="margin-left:16px; flex-shrink:0;">' +
          'View Job</a>' +
          '</div>' +
          '</div>';
      }).join('');
      ...
      if (el) el.innerHTML =
        '<div class="card"><div class="empty-state">' +
        '<p>Could not load recommendations. Please try again.</p></div></div>';
    }
  }